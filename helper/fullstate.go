...
func (fs *StateWithRwSets) GetRWSet() *rwset.RWSet {
	return fs.RwSets
}
...
func (fs *StateWithRwSets) GetCodeHash(addr common.Address) common.Hash {
	if fs.RwSets != nil {
		fs.RwSets.AddReadSet(addr, rwset.CODEHASH)
	}
...
func (fs *StateWithRwSets) HasSelfdestructed(addr common.Address) bool {
	if fs.RwSets != nil {
		fs.RwSets.AddReadSet(addr, rwset.ALIVE)
	}
...
func (fs *StateWithRwSets) SetRWSet(RwSets *rwset.RWSet) {
	fs.RwSets = RwSets
}
...
func (fs *StateWithRwSets) SetNonce(addr common.Address, nonce uint64) {
	if fs.RwSets != nil {
		fs.RwSets.AddWriteSet(addr, rwset.NONCE)
	}
...
func (fs *StateWithRwSets) SetCode(addr common.Address, code []byte) {
	if fs.RwSets != nil {
		fs.RwSets.AddWriteSet(addr, rwset.CODE)
		fs.RwSets.AddWriteSet(addr, rwset.CODEHASH)
	}
